...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) 
        ...
        // 1. Validar token primero antes de extraer cualquier claim
        if (!jwtService.isTokenValid(jwt)) {
            filterChain.doFilter(request, response);
            return;
        }
        
        // 2. Extraer rut solo si el token es válido
        final String rut = jwtService.extractRut(jwt);
        ...
        if (rut != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            ...
            // 4. Setear Authentication en SecurityContext
